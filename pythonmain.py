...
def main(stdscr, prompt):
    password = get_masked_input(stdscr, prompt)
    return password

# Usage example:
def get_password(stdscr, prompt):
    curses.curs_set(0)  # Hide cursor
    password = main(stdscr, prompt)
    stdscr.refresh()
    return password
...
def startprogram():
    ...
    asklogin = curses.wrapper(text_menu, menu)	
    if asklogin.lower() == 'login':
        logReason = 'attemptLogin'
        userName = curses.wrapper(get_input, "Please enter your username: ")
        password = curses.wrapper(get_password, "Please enter your password: ")
        try:
            logged, masterpassword = login(userName, password)
        except TypeError:
            logged = False
        if logged:
        ...
        userName = curses.wrapper(get_input, "Please enter your username: ")
        password = curses.wrapper(get_password, prompt="Please enter your password: ")
        passwordconfirm = curses.wrapper(get_password, prompt="Please confirm your password: ")
...
def clickable_text(normal_text, clickable_text, clickable_function, *args, **kwargs):
    def main(stdscr):
        ...
        def on_click():
            clickable_function(*args, **kwargs)
...
def addpass(user, masterpass):
    try:
        db = connect_to_database()
        mycursor = db.cursor()
        name = curses.wrapper(get_input, "Please enter the name of the password: ")
        action = curses.wrapper(text_menu, ['random password', 'custom password'])
        if action.lower() == 'random password':
            password = generate_password()
        else:
            password = curses.wrapper(get_password, prompt="Please enter the password: ")
        ...
        mycursor.execute(query, values)
        db.commit()
        log(user, 1, 'attemptAddPass')
        clickable_text(('This is your password: '+password+'    |   press a key to return'), "Click to copy to clipboard", lambda: copy(password))
        loggedin(user.lower(), masterpass)
    except mysql.connector.Error as error:
        log(user, 0, 'attemptAddPass')
        print(error)
            ...
            for row in myresult:
                ...
                while True:
                    if keyboard.is_pressed("s"):
                        ...
                        stdscr.refresh()
                        time.sleep(1)
                        log(user, 1, 'attemptViewPass')
                        loggedin(user.lower(), masterpass)
        else:
            log(user, 0, 'attemptViewPass')
