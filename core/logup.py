...
def login(username, password):
    ...
    myCursor.execute(query, (username.lower(),))
    userId = myCursor.fetchone()
    conn.close() # Close the connection
    return (True, masterPassword, logAttempt, userId) if login_succes == True else (False, None, logAttempt, userId) # Return the result: True if successful login, False otherwise
